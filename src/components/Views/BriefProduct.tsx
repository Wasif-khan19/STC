              ...
              <div className="flex">
                <span className="mr-3">Color: </span>
                {
                  product.size.map((item, index)=>(
                    <button key={index} className={`${size === item && 'ring-2 ring-indigo-600'}border-2 border-gray-300 rounded-full w-10 h-10 focus:outline-none`} onClick={()=> setsize(item)}>{item}</button>
                  ))
                }
              </div>
            </div>
